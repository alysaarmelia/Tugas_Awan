...
(async function() {
    ...
    try {
        ...
        // ── Stat cards ────────────────────────────────────────────
        document.getElementById('dashQuota').textContent     = `${storage.total_quota_gb} GB`;
        document.getElementById('dashQuotaUsed').textContent = `${storage.used_gb} / ${storage.total_quota_gb} GB used`;
        document.getElementById('dashUsed').textContent     = `${storage.used_gb} GB`;
        document.getElementById('dashUsagePercent').textContent = `${storage.usage_percent}%`;
        ...
        bar.style.width  = `${storage.usage_percent}%`;
        bar.className   = `h-3 rounded-full transition-all duration-500 ${UI.getProgressColor(storage.usage_percent)}`;
        document.getElementById('storageProgressLabel').textContent = `${storage.used_gb} GB / ${storage.total_quota_gb} GB`;
        document.getElementById('storageProgressPercent').textContent = `${storage.usage_percent}%`;

        // ── Quota breakdown ──────────────────────────────────────
        document.getElementById('dashBaseQuota').textContent = `${storage.base_quota_gb} GB`;
        document.getElementById('dashRented').textContent     = `+${storage.rented_gb} GB`;
        document.getElementById('dashRemaining').textContent  = `${storage.remaining_gb} GB`;
                ...
                <div class="flex items-start gap-3 p-3 bg-slate-50 rounded-lg">
                    ...
                    <div class="flex-1 min-w-0">
                        <div class="flex justify-between items-start">
                            ...
                            <span class="text-xs text-slate-400 flex-shrink-0">${UI.formatDate(log.created_at)}</span>
                        </div>
